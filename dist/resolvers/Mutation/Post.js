"use strict";
var __assign = (this && this.__assign) || function () {
    __assign = Object.assign || function(t) {
        for (var s, i = 1, n = arguments.length; i < n; i++) {
            s = arguments[i];
            for (var p in s) if (Object.prototype.hasOwnProperty.call(s, p))
                t[p] = s[p];
        }
        return t;
    };
    return __assign.apply(this, arguments);
};
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.postResolvers = void 0;
var canUserMutata_1 = require("../../utils/canUserMutata");
exports.postResolvers = {
    postCreate: function (_, _a, _b) {
        var _c = _a.post, title = _c.title, content = _c.content;
        var prisma = _b.prisma, userInfo = _b.userInfo;
        return __awaiter(void 0, void 0, void 0, function () {
            var _d;
            return __generator(this, function (_e) {
                switch (_e.label) {
                    case 0:
                        if (!userInfo) {
                            return [2 /*return*/, {
                                    userErrors: [{ message: "forbidden access, unauthenticated" }],
                                    post: null,
                                }];
                        }
                        if (!title || !content) {
                            return [2 /*return*/, {
                                    userErrors: [
                                        {
                                            message: "title or content is missing",
                                        },
                                    ],
                                    post: null,
                                }];
                        }
                        _d = {
                            userErrors: []
                        };
                        return [4 /*yield*/, prisma.post.create({
                                data: {
                                    title: title,
                                    content: content,
                                    authorId: userInfo.userId,
                                },
                            })];
                    case 1: return [2 /*return*/, (_d.post = _e.sent(),
                            _d)];
                }
            });
        });
    },
    postUpdate: function (_, _a, _b) {
        var postId = _a.postId, post = _a.post;
        var prisma = _b.prisma, userInfo = _b.userInfo;
        return __awaiter(void 0, void 0, void 0, function () {
            var error, content, title;
            var _c;
            return __generator(this, function (_d) {
                switch (_d.label) {
                    case 0:
                        if (!postId || !post) {
                            return [2 /*return*/, {
                                    userErrors: [
                                        {
                                            message: "postId or post is missing",
                                        },
                                    ],
                                    post: null,
                                }];
                        }
                        if (!userInfo) return [3 /*break*/, 2];
                        return [4 /*yield*/, (0, canUserMutata_1.CanUserMutatePost)({
                                userId: userInfo === null || userInfo === void 0 ? void 0 : userInfo.userId,
                                postId: +postId,
                                prisma: prisma,
                            })];
                    case 1:
                        error = _d.sent();
                        if (error) {
                            return [2 /*return*/, error];
                        }
                        _d.label = 2;
                    case 2:
                        content = post.content, title = post.title;
                        if (!content && !title) {
                            console.log(title, "iatch!!");
                            return [2 /*return*/, {
                                    userErrors: [
                                        {
                                            message: "content or title required",
                                        },
                                    ],
                                    post: null,
                                }];
                        }
                        _c = {
                            userErrors: []
                        };
                        return [4 /*yield*/, prisma.post.update({
                                where: {
                                    id: +postId,
                                },
                                data: __assign({}, post),
                            })];
                    case 3: return [2 /*return*/, (_c.post = _d.sent(),
                            _c)];
                }
            });
        });
    },
    postDelete: function (_, _a, _b) {
        var postId = _a.postId;
        var prisma = _b.prisma;
        return __awaiter(void 0, void 0, void 0, function () {
            var post;
            return __generator(this, function (_c) {
                switch (_c.label) {
                    case 0: return [4 /*yield*/, prisma.post.delete({
                            where: {
                                id: +postId,
                            },
                        })];
                    case 1:
                        post = _c.sent();
                        return [2 /*return*/, post];
                }
            });
        });
    },
    postPublish: function (_, _a, _b) {
        var postId = _a.postId, publish = _a.publish;
        var prisma = _b.prisma, userInfo = _b.userInfo;
        return __awaiter(void 0, void 0, void 0, function () {
            var error, newPost;
            return __generator(this, function (_c) {
                switch (_c.label) {
                    case 0:
                        if (!postId) {
                            return [2 /*return*/, {
                                    userErrors: [
                                        {
                                            message: "postId or post is missing",
                                        },
                                    ],
                                    post: null,
                                }];
                        }
                        if (!userInfo) return [3 /*break*/, 2];
                        return [4 /*yield*/, (0, canUserMutata_1.CanUserMutatePost)({
                                userId: userInfo === null || userInfo === void 0 ? void 0 : userInfo.userId,
                                postId: +postId,
                                prisma: prisma,
                            })];
                    case 1:
                        error = _c.sent();
                        if (error) {
                            return [2 /*return*/, error];
                        }
                        _c.label = 2;
                    case 2: return [4 /*yield*/, prisma.post.update({
                            where: {
                                id: +postId,
                            },
                            data: {
                                published: publish,
                            },
                        })];
                    case 3:
                        newPost = _c.sent();
                        return [2 /*return*/, {
                                userErrors: [],
                                post: newPost,
                            }];
                }
            });
        });
    },
};
